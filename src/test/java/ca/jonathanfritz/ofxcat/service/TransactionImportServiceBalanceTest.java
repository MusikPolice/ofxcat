...
class TransactionImportServiceBalanceTest extends AbstractDatabaseTest {
    ...
    void initialBalanceCalculationWithNoTransactions() {
        ...
        final OfxBalance ofxBalance =
                OfxBalance.newBuilder().setAmount(finalBalance).build();
        final List<OfxExport> ofxExports =
                Collections.singletonList(new OfxExport(ofxAccount, ofxBalance, Collections.emptyList()));
    ...
    void initialBalanceCalculationWithSingleCredit() {
        ...
        final OfxBalance ofxBalance =
                OfxBalance.newBuilder().setAmount(finalBalance).build();
        final List<OfxExport> ofxExports =
                Collections.singletonList(new OfxExport(ofxAccount, ofxBalance, Collections.singletonList(credit)));
        ...
        Assertions.assertEquals(
                expectedInitialBalance + creditAmount,
                finalBalance,
                0.01f,
    ...
    void initialBalanceCalculationWithSingleDebit() {
        ...
        final OfxBalance ofxBalance =
                OfxBalance.newBuilder().setAmount(finalBalance).build();
        final List<OfxExport> ofxExports =
                Collections.singletonList(new OfxExport(ofxAccount, ofxBalance, Collections.singletonList(debit)));
        ...
        // Additional verification: the math should work out
        Assertions.assertEquals(
                expectedInitialBalance + debitAmount,
                finalBalance,
                0.01f,
    ...
    void runningBalanceWithMultipleTransactions() {
        ...
        final OfxBalance ofxBalance =
                OfxBalance.newBuilder().setAmount(finalBalance).build();
        final List<OfxExport> ofxExports =
                Collections.singletonList(new OfxExport(ofxAccount, ofxBalance, Arrays.asList(debit1, credit, debit2)));
        ...
        result.sort((t1, t2) ->
                t1.getTransaction().getDate().compareTo(t2.getTransaction().getDate()));

        Assertions.assertEquals(
                900.0f, result.get(0).getTransaction().getBalance(), 0.01f, "First transaction balance should be $900");
        Assertions.assertEquals(
                950.0f,
                result.get(1).getTransaction().getBalance(),
                0.01f,
                "Second transaction balance should be $950");
        Assertions.assertEquals(
                750.0f, result.get(2).getTransaction().getBalance(), 0.01f, "Third transaction balance should be $750");
    }
    ...
    void transactionsProcessedInDateOrderRegardlessOfOfxOrder() {
        ...
        final List<OfxExport> ofxExports =
                Collections.singletonList(new OfxExport(ofxAccount, ofxBalance, Arrays.asList(jan3, jan1, jan2)));
        ...
        Assertions.assertEquals(
                900.0f, ct1.getTransaction().getBalance(), 0.01f, "Jan 1 transaction should have balance $900");
        Assertions.assertEquals(
                950.0f, ct2.getTransaction().getBalance(), 0.01f, "Jan 2 transaction should have balance $950");
        Assertions.assertEquals(
                750.0f, ct3.getTransaction().getBalance(), 0.01f, "Jan 3 transaction should have balance $750");
    }
    ...
    void transactionsSpanningYearBoundary() {
        ...
        final List<OfxExport> ofxExports =
                Collections.singletonList(new OfxExport(ofxAccount, ofxBalance, Arrays.asList(jan1, dec31)));
        ...
        Assertions.assertTrue(
                dec31Result.getTransaction().getBalance()
                        < jan1Result.getTransaction().getBalance(),
