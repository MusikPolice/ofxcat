...
class CategorizedTransactionDaoTest extends AbstractDatabaseTest {
    ...
    void insertSelectSuccessTest() {
        ...
        final CategorizedTransaction actual =
                categorizedTransactionDao.select(expected.getId()).orElse(null);
    ...
    void selectGroupByCategoryTest() {
        ...
        final CategorizedTransaction threeDaysAgo = categorizedTransactionDao
                .insert(new CategorizedTransaction(
                        TestUtils.createRandomTransaction(account, now.minusDays(3)), groceries))
                .orElse(null);
        final CategorizedTransaction twoDaysAgo = categorizedTransactionDao
                .insert(new CategorizedTransaction(
                        TestUtils.createRandomTransaction(account, now.minusDays(2)), groceries))
                .orElse(null);
        final CategorizedTransaction oneDayAgo = categorizedTransactionDao
                .insert(new CategorizedTransaction(
                        TestUtils.createRandomTransaction(account, now.minusDays(1)), restaurants))
                .orElse(null);

        // now we can get all transactions grouped by category
        Map<Category, List<CategorizedTransaction>> transactions =
                categorizedTransactionDao.selectGroupByCategory(now.minusDays(4), now);
    ...
    public void selectByCategoryTest() {
        ...
        for (int i = 0; i < 10; i++) {
            expected.add(categorizedTransactionDao
                    .insert(new CategorizedTransaction(TestUtils.createRandomTransaction(account), category))
                    .orElse(null));
        }
        ...
        categorizedTransactionDao.insert(
                new CategorizedTransaction(TestUtils.createRandomTransaction(account), UNKNOWN));

        // we can select all the transactions
        final LocalDate minDate = expected.stream()
                .map(Transaction::getDate)
                .min(LocalDate::compareTo)
                .orElse(null);
        final LocalDate maxDate = expected.stream()
                .map(Transaction::getDate)
    ...
    public void selectByCategoryWithoutDateFilterTest() {
        ...
        final Category restaurants =
                categoryDao.insert(new Category("RESTAURANTS")).orElse(null);
    ...
    public void findByDescriptionExactTest() throws SQLException {
        ...
        try (DatabaseTransaction t = new DatabaseTransaction(connection)) {
            ...
            final List<CategorizedTransaction> actual =
                    categorizedTransactionDao.findByDescription(t, expected.getDescription());
    ...
    public void selectByFitIdTest() {
        ...
        final CategorizedTransaction expected = categorizedTransactionDao
                .insert(new CategorizedTransaction(TestUtils.createRandomTransaction(account, fitId), category))
                .orElse(null);

        // and select it by fitId
        final CategorizedTransaction actual =
                categorizedTransactionDao.selectByFitId(fitId).orElse(null);
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void findUnlinkedTransfersTest() {
        ...
        final CategorizedTransaction source = categorizedTransactionDao
                .insert(new CategorizedTransaction(TestUtils.createRandomTransaction(sourceAccount), category))
                .orElse(null);
        final CategorizedTransaction sink = categorizedTransactionDao
                .insert(new CategorizedTransaction(TestUtils.createRandomTransaction(sinkAccount), category))
                .orElse(null);
        ...
        Assertions.assertEquals(
                source.getTransaction(), unlinkedTransfers.get(sourceAccount).get(0));
        Assertions.assertEquals(
                sink.getTransaction(), unlinkedTransfers.get(sinkAccount).get(0));
        ...
        Assertions.assertEquals(
                0, categorizedTransactionDao.findUnlinkedTransfers().size());
    }
    ...
    void hasTransactionsWithoutTokens_returnsFalseWhenAllTransactionsHaveTokens() throws SQLException {
        ...
        final Account account =
                accountDao.insert(TestUtils.createRandomAccount()).orElse(null);
        final CategorizedTransaction txn = categorizedTransactionDao
                .insert(new CategorizedTransaction(TestUtils.createRandomTransaction(account), category))
                .orElse(null);
    ...
    void selectWithoutTokens_returnsTransactionsWithoutTokens() throws SQLException {
        ...
        final CategorizedTransaction withTokens = categorizedTransactionDao
                .insert(new CategorizedTransaction(TestUtils.createRandomTransaction(account), category))
                .orElse(null);
        final CategorizedTransaction withoutTokens = categorizedTransactionDao
                .insert(new CategorizedTransaction(TestUtils.createRandomTransaction(account), category))
                .orElse(null);
