...
class MultiAccountIntegrationTest extends AbstractDatabaseTest {
    ...
    void transferBetweenCheckingAndSavings() {
        ...
        Account checking = accountDao.insert(TestUtils.createRandomAccount("Checking")).orElseThrow();
        Account savings = accountDao.insert(TestUtils.createRandomAccount("Savings")).orElseThrow();
        ...
        assertTrue(imported.stream().allMatch(t -> t.getCategory().equals(Category.TRANSFER)),
        ...
        assertEquals(sourceTransaction.getFitId(), storedTransfer.get().getSource().getFitId());
    ...
    void importFromMultipleAccountsSimultaneously() {
        ...
        Account checking = accountDao.insert(TestUtils.createRandomAccount("Primary Checking")).orElseThrow();
        Account savings = accountDao.insert(TestUtils.createRandomAccount("Savings")).orElseThrow();
        Account creditCard = accountDao.insert(TestUtils.createRandomAccount("Credit Card")).orElseThrow();
        ...
        List<OfxTransaction> savingsTransactions = List.of(
                createOfxTransaction("INTEREST", 5.50f, date, TransactionType.CREDIT)
        );
        ...
        // Create OFX exports
        List<OfxExport> ofxExports = List.of(
                new OfxExport(TestUtils.accountToOfxAccount(checking),
                        OfxBalance.newBuilder().setAmount(1500.00f).build(), checkingTransactions),
                new OfxExport(TestUtils.accountToOfxAccount(savings),
                        OfxBalance.newBuilder().setAmount(10005.50f).build(), savingsTransactions),
                new OfxExport(TestUtils.accountToOfxAccount(creditCard),
                        OfxBalance.newBuilder().setAmount(-620.00f).build(), creditCardTransactions)
        );
        ...
        long checkingCount = imported.stream()
                .filter(t -> t.getAccount().equals(checking))
                .count();
        long savingsCount = imported.stream()
                .filter(t -> t.getAccount().equals(savings))
                .count();
        long creditCardCount = imported.stream()
                .filter(t -> t.getAccount().equals(creditCard))
                .count();
    ...
    void transferMatchedAcrossSeparateImports() {
        ...
        List<OfxExport> checkingExport = List.of(
                new OfxExport(TestUtils.accountToOfxAccount(checking),
        ...
        List<OfxExport> savingsExport = List.of(
                new OfxExport(TestUtils.accountToOfxAccount(savings),
    ...
    void importFromNewAccountCreatesAccount() {
        ...
        List<OfxTransaction> transactions = List.of(
                createOfxTransaction("TEST MERCHANT", -50.00f, LocalDate.now(), TransactionType.DEBIT)
        );

        List<OfxExport> ofxExports = List.of(
                new OfxExport(newOfxAccount, OfxBalance.newBuilder().setAmount(500.00f).build(), transactions)
        );
        ...
        assertTrue(accounts.stream().anyMatch(a ->
                a.getAccountNumber().equals(newAccountNumber) && a.getBankId().equals(newBankId)),
